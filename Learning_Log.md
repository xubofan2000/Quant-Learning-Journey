...
| [x]  | [x]              | **Day 2** | `Learning_Vault/.../Week2/`                                            | 今日成功将 SQL 的窗口函数（Window Function）与偏移逻辑完美映射至 Pandas，利用 `.rolling()` 和 `.shift()` 实现了多周期移动均线（SMA）、收益率交叉校验及金叉死叉信号探测，彻底打通了金融时间序列的滚动特征基建。 |
